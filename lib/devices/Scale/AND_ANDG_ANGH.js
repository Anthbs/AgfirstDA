...
module.exports = function() {
    return new and_andg_andh();
}

function and_andg_andh() {
    this.settings = {
        device: {
            type: "Scale",
            model: "AND_ANDG_ANDH"
        ...
        regex: {
            get_value: regexp().start('ST').must(',').something().toRegExp(),
            unstable: regexp().start('US').must(',').something().toRegExp()
        },
        commands: {
            get_value: 'Q\r\n'
        },
    ...
    this.process_line = function(line) {
        if (this.settings.regex.get_value.test(line)) { //Stable
            var line_value = line.toString().split(',')[1].split(' ')[0];
            if (!isNaN(line_value)) {
        ...
        if (this.settings.regex.unstable.test(line)) { //Unstable
            this.get_value();
            return;
        }
    }.bind(this);
    ...
    this.get_result_object = function() {
        ...
        console.log("Scale Result: " + JSON.stringify(res_obj));
        return res_obj;
    }.bind(this);
