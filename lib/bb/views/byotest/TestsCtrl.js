(function() {
    angular.module("app.controllers").controller("TestsCtrl", function ($scope, $q, $log, $http, Test) {
	    var LoadAllTests = function() {
	    	return Test.query().$promise.then(function(tests) {
	    		$log.debug("LoadAllTests - Loaded {test_count} Tests".assign({ test_count: tests.length }));
	    ...
	    var LoadTest = function(test_id) {
	    	Test.get({ id: test_id }).$promise.then(function(test) {
	    		SelectTest(test);
	    	})
	    }
	    ...
	    var SaveTest = function()
		{
			GenerateCode().then(function() {
				...
				//Save to api
				APIService.SaveTest(Tests.Selected.Test.id, { id: Tests.Selected.Test.id, code: Tests.Selected.Test.code, actions: filteredActions });

				Tests.AvailableTests.remove(function(test) {
					return test.id == Tests.Selected.Test.id;
				});
				
				Tests.AvailableTests.push({ id: Tests.Selected.Test.id, code: Tests.Selected.Test.code, actions: filteredActions });
			});
		}

	    var SelectTest = function(test) {
	    	if(test != null) {
		    	$log.debug("SelectTest - Selected {test_id}".assign({ test_id: test.id }));
		    	Tests.Selected.Test = test;
		    } else {
		    	$log.debug("SelectTest - Selected {test_id}".assign({ test_id: null }));
		...
		{
			var template = 	'function {1}(obj) \{' +
							'this.FSM = new window.BaseFSM("{1}FSM","Start");' +
			...
			res += '\}';
			res += 'if(window.Tests == undefined) \{ window.Tests = \{\};\}';
			res += 'window.Tests["{1}"] = {1};'.assign(Tests.Selected.Test.id);
	    ...
	    var AddAction = function(action) {
            ...
            if(Tests.Selected.Test == null) {
            	Tests.Selected.Test = { id: "NewTest", code: "", actions: []};
            }
	    ...
	    var addInitialParameter = function() {
	    	...
	    	//Reset Fields
	    	Tests.Selected.Action.newParam = {name: "", value: ""};
	    	UpdateTestAvailableParameters();
	    }
	    ...
	    var addKey = function(action, obj) {
	    	if(obj != null && !obj.key.isBlank()) {
	    		...
	    		action.params.keys.push({ key: obj.key, isParam: obj.isParam });
	    	}
	    }
	    ...
	    var addStoreKey = function(action, obj) {
	    	if(obj != null && !obj.key.isBlank()) {
	    		...
	    		action.params.storeKeys.push({ key: obj.key, isParam: obj.isParam });
	    	}
	    }
