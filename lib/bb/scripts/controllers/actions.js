...
/**
 * @ngdoc function
 * @name netLabApp.controller:ActionsCtrl
 * @description
 * # ActionsCtrl
 * Controller of the netLabApp
 */
angular.module('netLabApp')
    .controller('ActionsCtrl', function($scope, $log, Action) {
        var LoadAllActions = function() {
            return Action.query().$promise.then(function(actions) {
                $log.debug("LoadAllActions - Loaded {action_count} Actions".assign({
        ...
        var LoadAction = function(action_id) {
            ...
            return Test.get({
                ...
                if (action != null && action.id != null) {
                    $log.debug("LoadAction - Loaded {action_id}".assign({
                        action_id: action.id
                    }));
                } else {
                    $log.debug("LoadAction - Couldn't Load {action_id}".assign({
                        action_id: action.id
                    }));
                }
            });
